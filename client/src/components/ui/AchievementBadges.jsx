    ...
    return (
        <motion.button
            ...
            className={`relative p-3 sm:p-4 rounded-xl border transition-all ${unlocked
                ...
                : 'bg-surface-200/50 border-surface-300 opacity-50 grayscale'
                }`}
        >
            {/* Icon */}
            ...
            {/* Name */}
            <h4 className={`font-semibold text-xs sm:text-sm ${unlocked ? 'text-surface-800' : 'text-surface-400'}`}>
                {badge.name}
            </h4>
            ...
            {!unlocked && (
                <div className="absolute top-2 right-2 text-surface-400 text-xs">🔒</div>
            )}
        </motion.button>
    );
}
    ...
    return (
        ...
        >
            <motion.div
                ...
                className={`bg-surface-100 rounded-2xl p-6 sm:p-8 max-w-sm w-full text-center border ${rarityStyle.border} ${rarityStyle.glow}`}
            >
                {/* Icon with animation */}
                <motion.div
                    initial={{ scale: 0 }}
                ...
                {/* Status */}
                <div className={`py-3 px-4 rounded-xl ${unlocked ? 'bg-success-500/10 border border-success-500/30' : 'bg-surface-200/50 border border-surface-300'}`}>
                    {unlocked ? (
        ...
        <AnimatePresence>
            ...
            >
                ...
                <motion.div
                    ...
                    className={`bg-surface-100 rounded-3xl p-8 max-w-md w-full text-center border-2 ${rarityStyle.border} shadow-2xl`}
                    style={{
                ...
                >
                    ...
                    <motion.h2
                        ...
                        className="text-2xl sm:text-3xl font-bold text-surface-800 mb-2"
                    >
                        {badge.name}
                    </motion.h2>
                    ...
                    <motion.p
                        ...
                        className="text-surface-500 mb-6"
                    >
                        {badge.description}
                    </motion.p>
...
export function BadgeSummary({ unlockedBadgeIds = [], onViewAll }) {
    ...
    return (
        <div className="bg-surface-100/80 backdrop-blur-sm rounded-xl lg:rounded-2xl p-4 sm:p-6 border border-surface-200/50">
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm sm:text-base font-semibold text-surface-800 flex items-center gap-2">
                    🏅 Achievements
                </h3>
            ...
            {unlockedBadges.length > 0 ? (
                <>
                    <div className="flex gap-2 mb-3 overflow-x-auto pb-2 hide-scrollbar">
                        ...
                        {unlockedCount > 5 && (
                            <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-surface-200/50 border border-surface-300 flex items-center justify-center text-xs text-surface-500">
                                +{unlockedCount - 5}
                            </div>
                        )}
                    </div>
                ...
                <div className="text-center py-4">
                    <p className="text-surface-400 text-sm mb-2">No badges unlocked yet</p>
                    <p className="text-surface-300 text-xs">Complete activities to earn badges!</p>
                </div>
            )}
        </div>
    );
