...
import CameraFeedback from '../../components/interview/CameraFeedback';
...
export default function InterviewRoom() {
    ...
    // Misbehavior/Cheating state
    const misbehaviorCountRef = useRef(0);
    ...
    useEffect(() => {
        if (tabSwitchCount > 0) {
            if (tabSwitchCount >= MAX_TAB_SWITCHES) {
                toast.error(`Warning: You have switched tabs ${tabSwitchCount} times. Your interview may be flagged.`);
            } else {
                toast.error(`Tab switch detected! (${tabSwitchCount}/${MAX_TAB_SWITCHES} allowed)`);
            }
        }
    }, [tabSwitchCount]);
    ...
    const handleSubmit = async () => {
        ...
        submitAnswerSocket(currentAnswer || voiceAnalysis?.transcription, bodyLanguageAnalysis);
    };
    ...
    const handleMisbehavior = (status) => {
        if (!isInterviewActive || isPaused || isProcessing) return;

        misbehaviorCountRef.current += 1;
        const newCount = misbehaviorCountRef.current;

        if (newCount >= 4) {
            toast.error('Cheating detected (multiple warnings ignored). The interview has been terminated.', { id: 'cheating-error', duration: 8000 });
            endMutation.mutate();
        } else {
            const actionText = status === 'looking-away' ? 'maintain eye contact with the camera' : 'sit still';
            toast.error(`Warning ${newCount}/3: Please ${actionText}. The interview will end if warnings are ignored.`, { id: 'posture-warning' });
        }
    };
        ...
        return (
            <div className="min-h-[60vh] flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-surface-500 text-xs sm:text-sm">Loading interview...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between gap-2 mb-3 sm:mb-5">
                <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                    ...
                    <span className="px-2 py-0.5 rounded-full bg-surface-200 text-surface-500 text-[10px] sm:text-xs capitalize">{session?.difficulty}</span>
                    <span className="text-surface-400 text-[10px] sm:text-xs">Q{questionIndex + 1}/{totalQuestions}</span>
                </div>
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                    ...
                    {/* Timer */}
                    <div className={`flex items-center gap-1 px-2 py-1 rounded-md text-[10px] sm:text-xs font-mono ${timer <= 30 ? 'bg-error-500/20 text-error-400' : 'bg-surface-200/80 text-surface-500'}`}>
                    ...
                    <button
                        ...
                        className="p-1.5 rounded-md bg-surface-200/50 text-surface-500 hover:bg-surface-300 transition-colors"
                    >
            ...
            {/* Progress Bar */}
            <div className="h-1 bg-surface-200/50 rounded-full mb-4 sm:mb-6 overflow-hidden">
                <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${totalQuestions > 0 ? (questionIndex / totalQuestions) * 100 : 0}%` }}
            ...
            <motion.div
                ...
                className="bg-surface-100/80 backdrop-blur-sm rounded-lg border border-surface-200/50 p-3 sm:p-4 mb-3 sm:mb-4"
            >
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 mb-1.5 sm:mb-2 flex-wrap">
                            ...
                            {currentQuestion?.difficulty && (
                                <span className="px-1.5 py-0.5 rounded bg-surface-200/80 text-surface-500 text-[9px] sm:text-xs capitalize">
                                    {currentQuestion.difficulty}
                                </span>
                            )}
                        </div>
                        <p className="text-xs sm:text-sm md:text-base text-surface-800 leading-relaxed">
                        ...
                        {currentQuestion?.expectedTopics?.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2 sm:mt-3">
                                {currentQuestion.expectedTopics.map((topic, i) => (
                                    <span key={i} className="px-1.5 py-0.5 bg-surface-200/50 rounded text-[9px] sm:text-xs text-surface-400">
                                        {topic}
                                    </span>
                                ))}
                            </div>
                ...
                {showFeedback && lastEvaluation ? (
                    ...
                    >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
                            <h3 className="text-lg sm:text-xl font-semibold text-surface-800 flex items-center gap-2">
                        ...
                        {/* Score Breakdown - 3 cols mobile, 6 cols desktop */}
                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 sm:gap-4 mb-4 sm:mb-6">
                            {['correctness', 'reasoning', 'communication', 'structure', 'confidence', 'postureAndPresence'].map((metric) => {
                                const scoreData = lastEvaluation.scores?.[metric];
                                if (!scoreData) return null; // Don't crash or render empty if missing
                                const score = typeof scoreData === 'object' ? (scoreData?.score ?? 0) : (scoreData ?? 0);
                                const displayName = metric === 'postureAndPresence' ? 'Presence' : metric;
                                return (
                                    <div key={metric} className="text-center">
                                        <div className={`text-lg sm:text-xl md:text-2xl font-bold ${getScoreColor(score)}`}>
                                            {score}%
                                        </div>
                                        <div className="text-surface-500 text-xs capitalize truncate" title={displayName}>{displayName}</div>
                                    </div>
                                );
                            })}
                        </div>
                                ...
                                <div>
                                    ...
                                    <ul className="space-y-1">
                                        {lastEvaluation.feedback.strengths.slice(0, 3).map((s, i) => (
                                            <li key={i} className="text-surface-600 text-sm flex items-start gap-2">
                        ...
                        {voiceAnalysis && (
                            <div className="p-3 sm:p-4 bg-surface-200/50 rounded-xl mb-4 sm:mb-6">
                                <h4 className="text-xs sm:text-sm font-medium text-surface-600 mb-3 flex items-center gap-2">
                                    <Volume2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                                    Voice Analysis
                                </h4>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 text-center text-sm">
                                    <div>
                                        <div className="text-base sm:text-lg font-semibold text-surface-800">{voiceAnalysis.confidence}%</div>
                                        <div className="text-surface-500 text-xs">Confidence</div>
                                    </div>
                                    <div>
                                        <div className="text-base sm:text-lg font-semibold text-surface-800">{voiceAnalysis.clarityScore}%</div>
                                        <div className="text-surface-500 text-xs">Clarity</div>
                                    </div>
                                    <div>
                                        <div className="text-base sm:text-lg font-semibold text-surface-800">{voiceAnalysis.wordsPerMinute}</div>
                                        <div className="text-surface-500 text-xs">WPM</div>
                                    </div>
                                    <div>
                                        <div className="text-base sm:text-lg font-semibold text-surface-800">{voiceAnalysis.hesitationCount}</div>
                                        <div className="text-surface-500 text-xs">Hesitations</div>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -15 }}
                        className="bg-surface-100/80 backdrop-blur-sm rounded-lg border border-surface-200/50 p-3 sm:p-4"
                    >
                        {/* Hidden strict tracking component */}
                        <CameraFeedback ref={cameraRef} onMisbehavior={handleMisbehavior} />
                        ...
                        {voiceAnalysis?.transcription && (
                            <div className="mb-3 p-2.5 bg-surface-200/50 rounded-lg">
                                <p className="text-[10px] text-surface-400 mb-0.5">Transcription:</p>
                                <p className="text-surface-800 text-xs sm:text-sm">{voiceAnalysis.transcription}</p>
                            </div>
                        )}
                        ...
                        <textarea
                            ...
                            className="w-full bg-white border border-surface-300/50 rounded-lg px-3 py-2.5 text-xs sm:text-sm text-surface-800 placeholder-surface-400 min-h-[120px] sm:min-h-[150px] resize-none mb-3 focus:outline-none focus:border-primary-500/50"
            ...
            {/* Paused Overlay */}
            {isPaused && (
                <div className="fixed inset-0 bg-surface-50/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="text-center">
                        <Pause className="w-10 h-10 sm:w-12 sm:h-12 text-primary-400 mx-auto mb-3" />
                        <h2 className="text-lg sm:text-xl font-bold text-surface-800 mb-1">Interview Paused</h2>
                        <p className="text-surface-500 text-xs sm:text-sm mb-4">Take your time. Click resume when ready.</p>
            ...
            {showTabWarning && (
                <div className="fixed inset-0 bg-surface-50/95 backdrop-blur-md flex items-center justify-center z-[60] p-4">
                    <motion.div
                        initial={{ scale: 0.9, opacity: 0 }}
                        animate={{ scale: 1, opacity: 1 }}
                        className="bg-surface-100 border border-error-500/30 rounded-xl p-6 max-w-md text-center shadow-2xl"
                    >
                        ...
                        <h2 className="text-xl font-bold text-surface-800 mb-2">Tab Switch Detected!</h2>
                        <p className="text-surface-500 text-sm mb-4">
                            Switching tabs or windows during an interview is not allowed.
                            This action has been recorded.
                        </p>
                        <div className="bg-surface-200/50 rounded-lg p-3 mb-4">
