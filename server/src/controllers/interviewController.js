...
export const startInterview = asyncHandler(async (req, res) => {
    ...
    // Subscription limits removed - unlimited interviews for all users
    ...
    // Interview count tracking removed - unlimited for all users
    ...
    // Generate first question (with fallback if AI is unavailable)
    let firstQuestion;
    try {
        firstQuestion = await geminiService.generateQuestion(sessionId, aiContext);
    } catch (error) {
        logger.warn(`AI question generation failed for session ${sessionId}, using fallback:`, error.message);
        // Fallback first questions by interview type
        const fallbackQuestions = {
            technical: {
                questionText: 'Can you explain the difference between a stack and a queue? When would you use each data structure?',
                questionType: 'technical',
                expectedTopics: ['data structures', 'stack', 'queue', 'LIFO', 'FIFO', 'use cases']
            },
            behavioral: {
                questionText: 'Tell me about a time when you had to work with a difficult team member. How did you handle the situation and what was the outcome?',
                questionType: 'open-ended',
                expectedTopics: ['conflict resolution', 'teamwork', 'communication', 'outcome']
            },
            hr: {
                questionText: 'Tell me about yourself. Walk me through your background and what interests you about this role.',
                questionType: 'open-ended',
                expectedTopics: ['background', 'motivation', 'career goals', 'relevant experience']
            },
            'system-design': {
                questionText: 'How would you design a URL shortening service like bit.ly? Consider the key components, data storage, and scalability.',
                questionType: 'scenario',
                expectedTopics: ['system architecture', 'database design', 'hashing', 'scalability', 'API design']
            }
        };
        firstQuestion = fallbackQuestions[interviewType] || fallbackQuestions.hr;
    }
...
export const submitAnswer = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        logger.warn(`AI evaluation failed for session ${sessionId}, using fallback:`, error.message);
        evaluation = {
            scores: {
                correctness: { score: 65, feedback: 'Answer evaluation is temporarily unavailable. Default score assigned.' },
    ...
    if (!isComplete) {
        ...
        if (!nextQuestion) {
            ...
            try {
                ...
                logger.warn(`AI question generation failed mid-interview for session ${sessionId}, using fallback:`, error.message);
                const fallbackPool = {
                    technical: [
