...
const responseSchema = new mongoose.Schema({
    ...
    bodyLanguageAnalysis: {
        ...
        fidgetCount: { type: Number, default: 0 },
        faceVisibleTime: { type: Number, default: 0 }
    },
    scores: {
        correctness: {
...
const interviewSessionSchema = new mongoose.Schema({
    ...
    overallScores: {
        ...
        confidence: { type: Number, default: 0 },
        postureAndPresence: { type: Number, default: 0 },
        overall: { type: Number, default: 0 }
    },
    analytics: {
...
interviewSessionSchema.methods.calculateOverallScores = function () {
    ...
    this.responses.forEach(response => {
        ...
        totals.postureAndPresence += response.scores.postureAndPresence?.score || 0;
    });
    ...
    this.overallScores = {
        ...
        postureAndPresence: Math.round(totals.postureAndPresence / count),
        overall: Math.round(
            (totals.correctness + totals.reasoning + totals.communication +
                totals.structure + totals.confidence + totals.postureAndPresence) / (count * 6)
        )
    };
};
